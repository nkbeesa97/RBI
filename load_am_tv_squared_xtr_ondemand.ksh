...
#set -vx
#--------------------------------------------------------------------------------------
# File      : load_am_tv_squared_xtr_ondemand.ksh
#
#
# Use       : exec_sf.ksh EXTRACTS NA load_am_tv_squared_xtr_ondemand.ksh 12 2021-01-07
#		exec_sf.ksh EXTRACTS NA load_am_tv_squared_xtr.ksh [the date number where any records greater than 12 days ago from the current date will be deleted from the table] [on demand date]
#
#
# AUDIT TRAIL
# Modified By
...
# 01/19/2020   Priscilla Thomas  2.0   AM2-816: Update date parameter
# 03/22/2021   Bhushan Newalkar	 3.0   SRDATA-15741: HF to load historical data based on client list and date range
# 08/15/2021   Priscilla Thomas  4.0   Add Trim to Customer Number for client list
# 10/06/2021   Hannah Zhu        5.0   default value 'Undefined' as BOOKED_SYSCODE_DESC when it's null
...
then
   ...
   echo Usage:  $0 \<V_MAX_NUMBER_HISTORY\>, \<V_START_DATE\>, and \<V_END_DATE\>
   echo
   echo
   exit 1
fi
...
v_client_list_file_name="${DIR_TEMP}/tvsquared_clientlist.tmp"
v_client_list_file_content=$(cat < $v_client_list_file_name)

if [ -z "$v_client_list_file_content" ]
then
	echo "File does not exist"
	exit 1
else
	echo "Loading data for the clients from the list"
fi
...
echo "V_MAX_NUMBER_HISTORY: ${V_MAX_NUMBER_HISTORY}"
echo "V_START_DATE              : ${v_start_dt}"
echo "V_END_DATE              : ${v_end_dt}"
echo
...
def main(argv):
  ...
  if (Action.errorLevel > 1):
    ...
    return
  executeSql([], [
    (f"""CREATE  temporary TABLE VT_XTR_ETL_PRCS_DT_DELETE AS
(
...
WHERE RNK >= ${V_MAX_NUMBER_HISTORY}
  ...
  executeSql([], [
	...
	SELECT e.cust_key AS Customer_Key, 
		...
		e.eclipse_regn_nm, 
		(c.RBI_SRC_CD||'-'|| CAST(TRIM(c.CUST_NBR) AS varchar(10))) AS Client_ID
  ...
  executeSql([], [
	...
	(select
	 ...
	 WHERE AD_EVNT_START_DT BETWEEN '${v_start_dt}' AND '${v_end_dt}'
	) F
  ...
  executeSql([], [
	...
	WHERE PATEF.AD_EVNT_START_DT BETWEEN '${v_start_dt}' AND '${v_end_dt}'
	GROUP BY
		 PATEF.CUST_KEY
		,T.CTRC_NBR
		,PATEF.ECLIPSE_REGN_NM
  ...
  executeSql([], [
    (f"""CREATE OR REPLACE TEMPORARY TABLE SPOT
	   ...
	   WHERE VRFY_FLG = 'Y'
	     AND AD.AD_EVNT_START_DT BETWEEN  '${v_start_dt}' AND '${v_end_dt}'""",
    [])
  ])
  if (Action.errorLevel > 1):
    print("MaxErrorLevel reached, exiting...")
  ...
  executeSql([], [
    (f"""CREATE OR REPLACE TEMPORARY TABLE ZF
      AS SELECT DISTINCT
	        ...
	        DSYS                    AS da_syscode,
	        ZIP_FACTOR
        FROM ${TD_DATABASE_AM_EDS}.AM_PARTNER_FACTOR_ZIPWEIGHT
  ...
  executeSql([], [
    ...
    FROM ${TD_DATABASE_AM_BI}.AM_SUBSCRIBER_DIM s
     LEFT JOIN ${TD_DATABASE_AM_BI}.AM_ZIP_DIM z
      ...
      WHERE s.eff_start_dt <= '${v_end_dt}' and s.eff_end_dt >=  '${v_start_dt}'
  ...
  executeSql([], [
    (f"""CREATE OR REPLACE TEMPORARY TABLE TT_LIN
      AS
		...
		WHERE L.AD_EVNT_START_DT BETWEEN  '${v_start_dt}' AND '${v_end_dt}'
  ...
  executeSql([], [
    (f"""CREATE OR REPLACE TEMPORARY TABLE
      PARTNER_SPOT_ZIP AS
	    ...
	    WHERE SI.IMP_FL = 0
	      AND AD_EVNT_START_DT BETWEEN  '${v_start_dt}' AND '${v_end_dt}'""",
    [])
  ])
  if (Action.errorLevel > 1):
    print("MaxErrorLevel reached, exiting...")
...
#-----5.14----- 
  ...
  #/*=======================================================================================================*/
  #-- Step 6: Get the genre from AM_PROGRAM_GENRE_DIM
  #/*=======================================================================================================*/
  executeSql([], [
  ...
  if (Action.errorLevel > 1):
    ...
    return
  executeSql([], [
    (f"""DELETE FROM ${TD_DATABASE_AM_XTR}.AM_TV_SQUARED_EXTRACT 
 WHERE AD_DATE BETWEEN '${v_start_dt}' AND '${v_end_dt}'
 AND CLIENT_ID IN (SELECT DISTINCT Client_ID FROM TMP_CLIENT_LIST)""",
    [])
  ])
  if (Action.errorLevel > 1):
    print("MaxErrorLevel reached, exiting...")
    return
  executeSql([], [
    (f"""DELETE FROM ${TD_DATABASE_AM_XTR}.AM_TV_SQUARED_EXTRACT WHERE UPPER(LD_SEQ_NBR) = UPPER('${DAILY_LD_NBR}')""",
    [])
  ])
  if (Action.errorLevel > 1):
    print("MaxErrorLevel reached, exiting...")
