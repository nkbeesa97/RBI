----------------------------------------------------------------------------------------------------------------------------------------------------
--  File:       load_xtr_tvsquared_exp.sql
-- 
--  C:\Git\SRD_AM_SNOWFLAKE\EXTRACTS\TVSQUARED\SNOWSQL\load_xtr_tvsquared_exp.sql
--  
--  Purpose:    This sql script is loading data into TVSquared Exposure extract table
-- 
...
--  --------      ------------       -------       --------------------------------
--  16-Jun-2021   Priscilla T.       1.1           SRDATA-15610: Initial Version sql
--  29-Nov-2021   Nanda Kumar B      2.0           SRDATA-16747: Added logic for column IMPRESSION_CONTRIBUTION
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- 
...
SELECT DISTINCT PROCESS_DT
FROM VT_XTR_PROCESS_DT_DELETE;
...
CREATE OR REPLACE TEMPORARY TABLE TMP_DATES AS (
    SELECT DISTINCT 
        B.CLNDR_WEEK_START_DT - INTERVAL '7  DAY' AS CLNDR_WEEK_START_DT
        , B.CLNDR_WEEK_END_DT
    FROM &{SF_DATABASE}.&{TD_DATABASE_AM_BI}.AM_CALENDAR_DIM A
    JOIN &{SF_DATABASE}.&{TD_DATABASE_AM_BI}.AM_CALENDAR_DIM B
...
CREATE OR REPLACE TEMPORARY TABLE LINEAR AS (
    ...
    WHERE A.AD_EVNT_START_DT BETWEEN (SELECT CLNDR_WEEK_START_DT FROM TMP_DATES) AND (SELECT CLNDR_WEEK_END_DT FROM TMP_DATES)
...
CREATE OR REPLACE TEMPORARY TABLE TVS_NEW_ADSE_STREAM AS(
    ...
    WHERE EVNT_UTC_DT BETWEEN (SELECT CLNDR_WEEK_START_DT FROM TMP_DATES) AND (SELECT CLNDR_WEEK_END_DT FROM TMP_DATES)
    AND SBSC_GUID_KEY <> -1
    AND EXCLUDE_REC = 0 
